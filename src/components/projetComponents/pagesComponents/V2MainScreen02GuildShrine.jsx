import React from "react";

export const cssFiles = ["v2-screen-shrine"];

export default function V2MainScreen02GuildShrine() {
  return (
    <div className="main world1">
      <div
        className="main-bg"
        style={{
          backgroundImage: "url('../img/images/back/new/09-River.jpg')",
        }}
      ></div>
      <div className="header">
        <div className="resource-panel">
          <div className="avatar-box-all">
            <div className="avatar-box-pic">
              <img src="img/hero0093-ava.jpg" loading="lazy" />
            </div>
            <div className="avatar-box-name">Dormidont</div>
          </div>
          <div className="resource-gold">
            <div className="value">6969K</div>
            <div className="value-bonus">+6969K%</div>
            <div className="resource-btn-add"></div>
            <div className="resource-pic">
              <img src="img/v2-res-gold.png" loading="lazy" />
            </div>
          </div>
          <div className="resource-vip lvl5">
            <div className="value-vip-diamond">6969K</div>
            <div className="value-vip-ticket">6969K</div>
            <div className="resource-btn-add"></div>
            <div className="resource-pic-vip">
              <div className="viplvl">VIP</div>
            </div>
          </div>
        </div>
        <div className="shop-chest-box-all">
          <div className="res-shop-btn">
            <div className="res-shop-btn-text">магазин</div>
          </div>
          <div className="res-chest-btn">
            <div className="res-chest-btn-text">Chests</div>
          </div>
        </div>
        <div className="singinout-btn out">
          <div className="singinout-btn-text">sign out</div>
        </div>
      </div>
      <div className="quest-box-all">
        <div className="quest-box">
          <div className="quest">
            <img className="icon" src="img/ic-abil-reborn.png" loading="lazy" />
          </div>
          <div className="quest">
            <img className="icon" src="img/ic-abil-reborn.png" loading="lazy" />
          </div>
          <div className="quest">
            <img className="icon" src="img/ic-abil-reborn.png" loading="lazy" />
          </div>
          <div className="quest">
            <img className="icon" src="img/ic-abil-reborn.png" loading="lazy" />
          </div>
          <div className="quest">
            <img className="icon" src="img/ic-abil-reborn.png" loading="lazy" />
          </div>
          <div className="quest">
            <img className="icon" src="img/ic-abil-reborn.png" loading="lazy" />
          </div>
          <div className="quest">
            <img className="icon" src="img/ic-abil-reborn.png" loading="lazy" />
          </div>
        </div>
        <div className="opt-panel-box">
          <div className="btn feedback"></div>
          <div className="btn statistic"></div>
          <div className="btn ru"></div>
        </div>
      </div>
      <div className="left-side">
        <div className="left-panel wide">
          <div className="tab-panel-box shrine">
            <div className="tab-panel-box-title">shrine</div>
            <div className="achive-box">
              <div className="achive-list">
                <div className="achiev-all-box">
                  <div className="achiev-name-box">
                    <div className="achiev-name">Path Finder</div>
                    <div className="achiev-star-box">
                      <div className="achiev-star">
                        <div className="achiev-num">2/15</div>
                      </div>
                    </div>
                    <div className="achiev-prize">gold 10T/+1050M</div>
                  </div>
                  <div className="achiev-pic-box">
                    <div className="achiev">
                      <div className="icon">
                        <img
                          className="img"
                          src="img/iconach/ic-ach-01@2x.jpg"
                          loading="lazy"
                        />
                      </div>
                    </div>
                  </div>
                  <div className="achiev-bonus-box">
                    <div className="achiev-bonus-name">bonus</div>
                    <div className="achiev-bonus-text-box">
                      <div className="achiev-bonus-text">+121545 dps</div>
                    </div>
                  </div>
                </div>
                <div className="achiev-all-box">
                  <div className="achiev-name-box">
                    <div className="achiev-name">Path Finder</div>
                    <div className="achiev-star-box">
                      <div className="achiev-star">
                        <div className="achiev-num">9/10</div>
                      </div>
                    </div>
                    <div className="achiev-prize">gold 10T/+1050M</div>
                  </div>
                  <div className="achiev-pic-box">
                    <div className="achiev">
                      <div className="icon">
                        <img
                          className="img"
                          src="img/iconach/ic-ach-02@2x.jpg"
                          loading="lazy"
                        />
                      </div>
                    </div>
                  </div>
                  <div className="achiev-bonus-box">
                    <div className="achiev-bonus-name">bonus</div>
                    <div className="achiev-bonus-text-box">
                      <div className="achiev-bonus-text">+121545 dps</div>
                    </div>
                  </div>
                </div>
                <div className="achiev-all-box">
                  <div className="achiev-name-box">
                    <div className="achiev-name">Path Finder</div>
                    <div className="achiev-star-box">
                      <div className="achiev-star">
                        <div className="achiev-num">15/20</div>
                      </div>
                    </div>
                    <div className="achiev-prize">gold 10T/+1050M</div>
                  </div>
                  <div className="achiev-pic-box">
                    <div className="achiev">
                      <div className="icon">
                        <img
                          className="img"
                          src="img/iconach/ic-ach-03@2x.jpg"
                          loading="lazy"
                        />
                      </div>
                    </div>
                  </div>
                  <div className="achiev-bonus-box">
                    <div className="achiev-bonus-name">bonus</div>
                    <div className="achiev-bonus-text-box">
                      <div className="achiev-bonus-text">+121545 dps</div>
                    </div>
                  </div>
                </div>
                <div className="achiev-all-box">
                  <div className="achiev-name-box">
                    <div className="achiev-name">Path Finder</div>
                    <div className="achiev-star-box">
                      <div className="achiev-star">
                        <div className="achiev-num">12/20</div>
                      </div>
                    </div>
                    <div className="achiev-prize">gold 10T/+1050M</div>
                  </div>
                  <div className="achiev-pic-box">
                    <div className="achiev lvl01">
                      <div className="icon">
                        <img
                          className="img"
                          src="img/iconach/ic-ach-04@2x.jpg"
                          loading="lazy"
                        />
                      </div>
                    </div>
                  </div>
                  <div className="achiev-bonus-box">
                    <div className="achiev-bonus-name">bonus</div>
                    <div className="achiev-bonus-text-box">
                      <div className="achiev-bonus-text">+121545 dps</div>
                    </div>
                  </div>
                </div>
                <div className="achiev-all-box">
                  <div className="achiev-name-box">
                    <div className="achiev-name">Path Finder</div>
                    <div className="achiev-star-box">
                      <div className="achiev-star">
                        <div className="achiev-num">1/8</div>
                      </div>
                    </div>
                    <div className="achiev-prize">gold 10T/+1050M</div>
                  </div>
                  <div className="achiev-pic-box">
                    <div className="achiev">
                      <div className="icon">
                        <img
                          className="img"
                          src="img/iconach/ic-ach-05@2x.jpg"
                          loading="lazy"
                        />
                      </div>
                    </div>
                  </div>
                  <div className="achiev-bonus-box">
                    <div className="achiev-bonus-name">bonus</div>
                    <div className="achiev-bonus-text-box">
                      <div className="achiev-bonus-text">+121545 dps</div>
                    </div>
                  </div>
                </div>
                <div className="achiev-all-box">
                  <div className="achiev-name-box">
                    <div className="achiev-name">Path Finder</div>
                    <div className="achiev-star-box">
                      <div className="achiev-star">
                        <div className="achiev-num">1/8</div>
                      </div>
                    </div>
                    <div className="achiev-prize">gold 10T/+1050M</div>
                  </div>
                  <div className="achiev-pic-box">
                    <div className="achiev">
                      <div className="icon">
                        <img
                          className="img"
                          src="img/iconach/ic-ach-06@2x.jpg"
                          loading="lazy"
                        />
                      </div>
                    </div>
                  </div>
                  <div className="achiev-bonus-box">
                    <div className="achiev-bonus-name">bonus</div>
                    <div className="achiev-bonus-text-box">
                      <div className="achiev-bonus-text">+121545 dps</div>
                    </div>
                  </div>
                </div>
                <div className="achiev-all-box">
                  <div className="achiev-name-box">
                    <div className="achiev-name">Path Finder</div>
                    <div className="achiev-star-box">
                      <div className="achiev-star">
                        <div className="achiev-num">1/8</div>
                      </div>
                    </div>
                    <div className="achiev-prize">gold 10T/+1050M</div>
                  </div>
                  <div className="achiev-pic-box">
                    <div className="achiev">
                      <div className="icon">
                        <img
                          className="img"
                          src="img/iconach/ic-ach-07@2x.jpg"
                          loading="lazy"
                        />
                      </div>
                    </div>
                  </div>
                  <div className="achiev-bonus-box">
                    <div className="achiev-bonus-name">bonus</div>
                    <div className="achiev-bonus-text-box">
                      <div className="achiev-bonus-text">+121545 dps</div>
                    </div>
                  </div>
                </div>
                <div className="achiev-all-box">
                  <div className="achiev-name-box">
                    <div className="achiev-name">Path Finder</div>
                    <div className="achiev-star-box">
                      <div className="achiev-star">
                        <div className="achiev-num">1/8</div>
                      </div>
                    </div>
                    <div className="achiev-prize">gold 10T/+1050M</div>
                  </div>
                  <div className="achiev-pic-box">
                    <div className="achiev">
                      <div className="icon">
                        <img
                          className="img"
                          src="img/iconach/ic-ach-08@2x.jpg"
                          loading="lazy"
                        />
                      </div>
                    </div>
                  </div>
                  <div className="achiev-bonus-box">
                    <div className="achiev-bonus-name">bonus</div>
                    <div className="achiev-bonus-text-box">
                      <div className="achiev-bonus-text">+121545 dps</div>
                    </div>
                  </div>
                </div>
                <div className="achiev-all-box">
                  <div className="achiev-name-box">
                    <div className="achiev-name">Path Finder</div>
                    <div className="achiev-star-box">
                      <div className="achiev-star">
                        <div className="achiev-num">1/8</div>
                      </div>
                    </div>
                    <div className="achiev-prize">gold 10T/+1050M</div>
                  </div>
                  <div className="achiev-pic-box">
                    <div className="achiev">
                      <div className="icon">
                        <img
                          className="img"
                          src="img/iconach/ic-ach-09@2x.jpg"
                          loading="lazy"
                        />
                      </div>
                    </div>
                  </div>
                  <div className="achiev-bonus-box">
                    <div className="achiev-bonus-name">bonus</div>
                    <div className="achiev-bonus-text-box">
                      <div className="achiev-bonus-text">+121545 dps</div>
                    </div>
                  </div>
                </div>
                <div className="achiev-all-box">
                  <div className="achiev-name-box">
                    <div className="achiev-name">Path Finder</div>
                    <div className="achiev-star-box">
                      <div className="achiev-star">
                        <div className="achiev-num">1/8</div>
                      </div>
                    </div>
                    <div className="achiev-prize">gold 10T/+1050M</div>
                  </div>
                  <div className="achiev-pic-box">
                    <div className="achiev">
                      <div className="icon">
                        <img
                          className="img"
                          src="img/iconach/ic-ach-10@2x.jpg"
                          loading="lazy"
                        />
                      </div>
                    </div>
                  </div>
                  <div className="achiev-bonus-box">
                    <div className="achiev-bonus-name">bonus</div>
                    <div className="achiev-bonus-text-box">
                      <div className="achiev-bonus-text">+121545 dps</div>
                    </div>
                  </div>
                </div>
                <div className="achiev-all-box">
                  <div className="achiev-name-box">
                    <div className="achiev-name">Path Finder</div>
                    <div className="achiev-star-box">
                      <div className="achiev-star">
                        <div className="achiev-num">1/8</div>
                      </div>
                    </div>
                    <div className="achiev-prize">gold 10T/+1050M</div>
                  </div>
                  <div className="achiev-pic-box">
                    <div className="achiev">
                      <div className="icon">
                        <img
                          className="img"
                          src="img/iconach/ic-ach-11@2x.jpg"
                          loading="lazy"
                        />
                      </div>
                    </div>
                  </div>
                  <div className="achiev-bonus-box">
                    <div className="achiev-bonus-name">bonus</div>
                    <div className="achiev-bonus-text-box">
                      <div className="achiev-bonus-text">+121545 dps</div>
                    </div>
                  </div>
                </div>
                <div className="achiev-all-box">
                  <div className="achiev-name-box">
                    <div className="achiev-name">Path Finder</div>
                    <div className="achiev-star-box">
                      <div className="achiev-star">
                        <div className="achiev-num">1/8</div>
                      </div>
                    </div>
                    <div className="achiev-prize">gold 10T/+1050M</div>
                  </div>
                  <div className="achiev-pic-box">
                    <div className="achiev">
                      <div className="icon">
                        <img
                          className="img"
                          src="img/iconach/ic-ach-12@2x.jpg"
                          loading="lazy"
                        />
                      </div>
                    </div>
                  </div>
                  <div className="achiev-bonus-box">
                    <div className="achiev-bonus-name">bonus</div>
                    <div className="achiev-bonus-text-box">
                      <div className="achiev-bonus-text">+121545 dps</div>
                    </div>
                  </div>
                </div>
                <div className="achiev-all-box">
                  <div className="achiev-name-box">
                    <div className="achiev-name">Path Finder</div>
                    <div className="achiev-star-box">
                      <div className="achiev-star">
                        <div className="achiev-num">1/8</div>
                      </div>
                    </div>
                    <div className="achiev-prize">gold 10T/+1050M</div>
                  </div>
                  <div className="achiev-pic-box">
                    <div className="achiev">
                      <div className="icon">
                        <img
                          className="img"
                          src="img/iconach/ic-ach-13@2x.jpg"
                          loading="lazy"
                        />
                      </div>
                    </div>
                  </div>
                  <div className="achiev-bonus-box">
                    <div className="achiev-bonus-name">bonus</div>
                    <div className="achiev-bonus-text-box">
                      <div className="achiev-bonus-text">+121545 dps</div>
                    </div>
                  </div>
                </div>
                <div className="achiev-all-box">
                  <div className="achiev-name-box">
                    <div className="achiev-name">Path Finder</div>
                    <div className="achiev-star-box">
                      <div className="achiev-star">
                        <div className="achiev-num">1/8</div>
                      </div>
                    </div>
                    <div className="achiev-prize">gold 10T/+1050M</div>
                  </div>
                  <div className="achiev-pic-box">
                    <div className="achiev">
                      <div className="icon">
                        <img
                          className="img"
                          src="img/iconach/ic-ach-14@2x.jpg"
                          loading="lazy"
                        />
                      </div>
                    </div>
                  </div>
                  <div className="achiev-bonus-box">
                    <div className="achiev-bonus-name">bonus</div>
                    <div className="achiev-bonus-text-box">
                      <div className="achiev-bonus-text">+121545 dps</div>
                    </div>
                  </div>
                </div>
              </div>
            </div>

            <div className="btn-close-x"></div>
          </div>
        </div>
        <div className="tabs-all-box">
          <div className="tabs-all selected">
            <div className="tabs-name">Heroes</div>
            <div className="tabs-pic">
              <img src="img/tab-01.png" loading="lazy" />
            </div>
          </div>
          <div className="tabs-all">
            <div className="tabs-name">Album</div>
            <div className="tabs-pic">
              <img src="img/tab-05.png" loading="lazy" />
            </div>
          </div>
          <div className="tabs-all">
            <div className="tabs-name">Guild</div>
            <div className="tabs-pic">
              <img src="img/tab-08.png" loading="lazy" />
            </div>
          </div>
          <div className="tabs-all">
            <div className="tabs-name">Rating</div>
            <div className="tabs-pic">
              <img src="img/tab-02.png" loading="lazy" />
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}

import React from "react";
import { Helmet, HelmetProvider } from "react-helmet-async";
import ScreenFortunaRoundMainStyles from "../styleComponents/ScreenFortunaRoundMain.jsx";

const MainScreen16FortunaRoundMainpopComponent = ({ pageName }) => {
  return (
    <HelmetProvider>
      <Helmet>
        <style>
          {`
            #root-${pageName} {
              ${ScreenFortunaRoundMainStyles.styles}
            }
          `}
        </style>
      </Helmet>
      <div className="main world1">
        <div className="main-bg"></div>
        <div className="popup-layer" style={{ display: "block" }}>
          <div className="screen-blend-55" style={{ display: "block" }}></div>

          <div
            className="m-popup fortuna-main-round"
            style={{ display: "block" }}
          >
            <div className="title">wheel of fortune</div>
            <div className="fortuna-main-box">
              <div className="color-btn blue rating">
                <div className="color-btn-text">Rating</div>
              </div>
              <div className="push-timer">
                Next free push in:
                <span className="push-timer-time">23:59:59</span>
              </div>
              <div className="place">
                <div className="place-text">
                  Your place:
                  <div>Not in rating</div>
                </div>
              </div>
              <div className="fortuna-wheel-all">
                <div className="fortuna-wheel-box">
                  <div className="fortuna-wheel b1 good girl doublespin">
                    <div className="fortuna-prize">
                      <img
                        className="prize-pic"
                        src="img/icons/ic-abil-185.jpg"
                        loading="lazy"
                      />
                      <div className="fortuna-check-num res-amount">10</div>
                      <div className="fortuna-check-num">
                        {/* react-text: 613 */}x{/* /react-text */}
                        {/* react-text: 614 */}2{/* /react-text */}
                      </div>
                    </div>
                  </div>
                  <div className="fortuna-wheel b2 winner selected cheked">
                    <div className="fortuna-prize">
                      <img
                        className="prize-pic"
                        src="img/ms-stone-pink.png"
                        loading="lazy"
                      />
                      <div className="fortuna-check-num res-amount">5.77M</div>
                    </div>
                  </div>
                  <div className="fortuna-wheel b3">
                    <div className="fortuna-prize">
                      <img
                        className="prize-pic"
                        src="img/icons/ic-abil-185.jpg"
                        loading="lazy"
                      />
                    </div>
                  </div>
                  <div className="fortuna-wheel b4">
                    <div className="fortuna-prize">
                      <img
                        className="prize-pic"
                        src="img/icons/ic-abil-185.jpg"
                        loading="lazy"
                      />
                    </div>
                  </div>
                  <div className="fortuna-wheel b5">
                    <div className="fortuna-prize">
                      <img
                        className="prize-pic"
                        src="img/icons/ic-abil-185.jpg"
                        loading="lazy"
                      />
                    </div>
                  </div>
                  <div className="fortuna-wheel b6">
                    <div className="fortuna-prize">
                      <img
                        className="prize-pic"
                        src="img/icons/ic-abil-185.jpg"
                        loading="lazy"
                      />
                    </div>
                  </div>
                  <div className="fortuna-wheel b7">
                    <div className="fortuna-prize">
                      <img
                        className="prize-pic"
                        src="img/icons/ic-abil-185.jpg"
                        loading="lazy"
                      />
                    </div>
                  </div>
                  <div className="fortuna-wheel b8">
                    <div className="fortuna-prize">
                      <img
                        className="prize-pic"
                        src="img/icons/ic-abil-185.jpg"
                        loading="lazy"
                      />
                    </div>
                  </div>
                  <div className="fortuna-wheel b9">
                    <div className="fortuna-prize">
                      <img
                        className="prize-pic"
                        src="img/icons/ic-abil-185.jpg"
                        loading="lazy"
                      />
                    </div>
                  </div>
                  <div className="fortuna-wheel b10">
                    <div className="fortuna-prize">
                      <img
                        className="prize-pic"
                        src="img/icons/ic-abil-185.jpg"
                        loading="lazy"
                      />
                    </div>
                  </div>
                  <div className="fortuna-wheel b11">
                    <div className="fortuna-prize">
                      <img
                        className="prize-pic"
                        src="img/icons/ic-abil-185.jpg"
                        loading="lazy"
                      />
                    </div>
                  </div>
                  <div className="fortuna-wheel b12">
                    <div className="fortuna-prize">
                      <img
                        className="prize-pic"
                        src="img/icons/ic-abil-185.jpg"
                        loading="lazy"
                      />
                    </div>
                  </div>
                  <div className="fortuna-wheel b13">
                    <div className="fortuna-prize">
                      <img
                        className="prize-pic"
                        src="img/icons/ic-abil-185.jpg"
                        loading="lazy"
                      />
                    </div>
                  </div>
                  <div className="fortuna-wheel b14">
                    <div className="fortuna-prize">
                      <img
                        className="prize-pic"
                        src="img/icons/ic-abil-185.jpg"
                        loading="lazy"
                      />
                    </div>
                  </div>
                  <div className="fortuna-wheel b15">
                    <div className="fortuna-prize">
                      <img
                        className="prize-pic"
                        src="img/icons/ic-abil-185.jpg"
                        loading="lazy"
                      />
                    </div>
                  </div>
                  <div className="fortuna-wheel b16">
                    <div className="fortuna-prize">
                      <img
                        className="prize-pic"
                        src="img/icons/ic-abil-185.jpg"
                        loading="lazy"
                      />
                    </div>
                  </div>
                  <div className="fortuna-wheel b17">
                    <div className="fortuna-prize">
                      <img
                        className="prize-pic"
                        src="img/icons/ic-abil-185.jpg"
                        loading="lazy"
                      />
                    </div>
                  </div>
                  <div className="fortuna-wheel b18">
                    <div className="fortuna-prize">
                      <img
                        className="prize-pic"
                        src="img/icons/ic-abil-185.jpg"
                        loading="lazy"
                      />
                    </div>
                  </div>
                  <div className="fortuna-wheel b19">
                    <div className="fortuna-prize">
                      <img
                        className="prize-pic"
                        src="img/icons/ic-abil-185.jpg"
                        loading="lazy"
                      />
                    </div>
                  </div>
                  <div className="fortuna-wheel b20">
                    <div className="fortuna-prize">
                      <img
                        className="prize-pic"
                        src="img/icons/ic-abil-185.jpg"
                        loading="lazy"
                      />
                    </div>
                  </div>
                  <div className="fortuna-wheel b21">
                    <div className="fortuna-prize">
                      <img
                        className="prize-pic"
                        src="img/icons/ic-abil-185.jpg"
                        loading="lazy"
                      />
                    </div>
                  </div>
                  <div className="fortuna-wheel b22">
                    <div className="fortuna-prize">
                      <img
                        className="prize-pic"
                        src="img/icons/ic-abil-185.jpg"
                        loading="lazy"
                      />
                    </div>
                  </div>
                  <div className="fortuna-wheel b23">
                    <div className="fortuna-prize">
                      <img
                        className="prize-pic"
                        src="img/icons/ic-abil-185.jpg"
                        loading="lazy"
                      />
                    </div>
                  </div>
                  <div className="fortuna-wheel b24">
                    <div className="fortuna-prize">
                      <img
                        className="prize-pic"
                        src="img/icons/ic-abil-185.jpg"
                        loading="lazy"
                      />
                    </div>
                  </div>
                  <div className="fortuna-wheel b25">
                    <div className="fortuna-prize">
                      <img
                        className="prize-pic"
                        src="img/icons/ic-abil-185.jpg"
                        loading="lazy"
                      />
                    </div>
                  </div>
                </div>
                <div className="fortuna-wheel-center">
                  <div className="fortuna-btn-box">
                    <div className="color-btn green">
                      <div className="color-btn-text">
                        push x1
                        <span className="ff-but"></span>
                        9999
                      </div>
                    </div>
                    <div className="color-btn green x10">
                      <div className="color-btn-text">
                        push x10
                        <span className="ff-but"></span>
                        9999
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="fortuna-main-controls-box">
                <div className="rjackpot-box collected">
                  <div className="rname-box">Jackpot</div>
                  <div className="fortuna-reward-box-all">
                    <div className="fortuna-reward-box claim-btn">
                      <div className="fortuna-reward-bg">
                        <div className="fortuna-reward">
                          <img
                            src="img/images/superhero/suphero-240/x1/avatar/240sh-ava-1.jpg"
                            loading="lazy"
                          />
                        </div>
                      </div>
                      <div className="fortuna-reward-claim-btn">6969k</div>
                    </div>
                    <div className="fortuna-reward-box">
                      <div className="fortuna-reward-bg">
                        <div className="fortuna-reward">
                          <img
                            src="img/images/superhero/suphero-241/x1/avatar/241sh-ava-1.jpg"
                            loading="lazy"
                          />
                        </div>
                      </div>
                      <div className="fortuna-reward-claim-btn">6969k</div>
                    </div>
                    <div className="fortuna-reward-box">
                      <div className="fortuna-reward-bg">
                        <div className="fortuna-reward">
                          <img
                            src="img/images/superhero/suphero-242/x1/avatar/242sh-ava-1.jpg"
                            loading="lazy"
                          />
                        </div>
                      </div>
                      <div className="fortuna-reward-claim-btn">6969k</div>
                    </div>
                    <div className="fortuna-reward-box">
                      <div className="fortuna-reward-bg">
                        <div className="fortuna-reward">
                          <img src="img/change-ic-gold.png" loading="lazy" />
                        </div>
                      </div>
                      <div className="fortuna-reward-claim-btn">6969k</div>
                    </div>
                    <div className="fortuna-reward-box">
                      <div className="fortuna-reward-bg">
                        <div className="fortuna-reward">
                          <img src="img/change-ic-gold.png" loading="lazy" />
                        </div>
                      </div>
                      <div className="fortuna-reward-claim-btn">6969k</div>
                    </div>
                    <div className="fortuna-reward-box">
                      <div className="fortuna-reward-bg">
                        <div className="fortuna-reward">
                          <img src="img/ms-stone-pink.png" loading="lazy" />
                        </div>
                      </div>
                      <div className="fortuna-reward-claim-btn">6969k</div>
                    </div>
                    <div className="fortuna-reward-box">
                      <div className="fortuna-reward-bg">
                        <div className="fortuna-reward">
                          <img src="img/ms-stone-pink.png" loading="lazy" />
                        </div>
                      </div>
                      <div className="fortuna-reward-claim-btn">6969k</div>
                    </div>
                  </div>
                  <div className="fortuna-reward-box-all second">
                    <div className="fortuna-reward-box claim-btn">
                      <div className="fortuna-reward-bg">
                        <div className="fortuna-reward">
                          <img
                            src="img/images/superhero/suphero-240/x1/avatar/240sh-ava-1.jpg"
                            loading="lazy"
                          />
                        </div>
                      </div>
                      <div className="fortuna-reward-claim-btn">6969k</div>
                    </div>
                    <div className="fortuna-reward-box">
                      <div className="fortuna-reward-bg">
                        <div className="fortuna-reward">
                          <img
                            src="img/images/superhero/suphero-241/x1/avatar/241sh-ava-1.jpg"
                            loading="lazy"
                          />
                        </div>
                      </div>
                      <div className="fortuna-reward-claim-btn">6969k</div>
                    </div>
                    <div className="fortuna-reward-box">
                      <div className="fortuna-reward-bg">
                        <div className="fortuna-reward">
                          <img
                            src="img/images/superhero/suphero-242/x1/avatar/242sh-ava-1.jpg"
                            loading="lazy"
                          />
                        </div>
                      </div>
                      <div className="fortuna-reward-claim-btn">6969k</div>
                    </div>
                    <div className="fortuna-reward-box">
                      <div className="fortuna-reward-bg">
                        <div className="fortuna-reward">
                          <img src="img/change-ic-gold.png" loading="lazy" />
                        </div>
                      </div>
                      <div className="fortuna-reward-claim-btn">6969k</div>
                    </div>
                    <div className="fortuna-reward-box">
                      <div className="fortuna-reward-bg">
                        <div className="fortuna-reward">
                          <img src="img/change-ic-gold.png" loading="lazy" />
                        </div>
                      </div>
                      <div className="fortuna-reward-claim-btn">6969k</div>
                    </div>
                    <div className="fortuna-reward-box">
                      <div className="fortuna-reward-bg">
                        <div className="fortuna-reward">
                          <img src="img/ms-stone-pink.png" loading="lazy" />
                        </div>
                      </div>
                      <div className="fortuna-reward-claim-btn">6969k</div>
                    </div>
                    <div className="fortuna-reward-box">
                      <div className="fortuna-reward-bg">
                        <div className="fortuna-reward">
                          <img src="img/ms-stone-pink.png" loading="lazy" />
                        </div>
                      </div>
                      <div className="fortuna-reward-claim-btn">6969k</div>
                    </div>
                    <div className="wf-jp-2-cloaker">
                      Buy pack to unlock double rewards
                      <div className="color-btn">
                        <div className="color-btn-text">Show me</div>
                      </div>
                    </div>
                  </div>
                  <div className="rjackpot-box-collected-label">
                    <div className="rjackpot-box-collected-label-text">
                      collected
                    </div>
                  </div>
                </div>
                <div className="rrevard-box">
                  <div className="rname-box">Reward for rolls</div>
                  <div className="fortuna-reward-btn color-btn blue">
                    show all
                  </div>
                  <div className="fortuna-prgbar-box">
                    <div className="f2-prgbar-empty">
                      <div
                        className="f2-prgbar-full"
                        style={{
                          width: "15%",
                        }}
                      ></div>
                      <div className="f2-prgbar-text">100/1500</div>
                    </div>
                  </div>
                  <div className="fortuna-reward-box-all">
                    <div className="fortuna-reward-box claim-btn">
                      <div className="fortuna-reward-bg">
                        <div className="fortuna-reward">
                          <img src="img/ms-stone-pink.png" loading="lazy" />
                        </div>
                      </div>
                      <div className="fortuna-reward-claim-btn">med x 4</div>
                    </div>
                    <div className="fortuna-reward-box">
                      <div className="fortuna-reward-bg">
                        <div className="fortuna-reward">
                          <img src="img/ms-stone-pink.png" loading="lazy" />
                        </div>
                      </div>
                      <div className="fortuna-reward-claim-btn">10K%</div>
                    </div>
                    <div className="fortuna-reward-box">
                      <div className="fortuna-reward-bg">
                        <div className="fortuna-reward">
                          <img src="img/ms-stone-pink.png" loading="lazy" />
                        </div>
                      </div>
                      <div className="fortuna-reward-claim-btn">35</div>
                    </div>
                    <div className="fortuna-reward-box">
                      <div className="fortuna-reward-bg">
                        <div className="fortuna-reward">
                          <img src="img/ms-stone-pink.png" loading="lazy" />
                        </div>
                      </div>
                      <div className="fortuna-reward-claim-btn">15</div>
                    </div>
                    <div className="fortuna-reward-box">
                      <div className="fortuna-reward-bg">
                        <div className="fortuna-reward">
                          <img src="img/ms-stone-pink.png" loading="lazy" />
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="rrules-box">
                  <div className="rules-text">
                    <div>
                      1. During the time of the event, you can spin the Great
                      Wheel of Fortune for diamonds.
                    </div>
                    <div>
                      2. Each use of the Push button gives a prize and increases
                      points counter.
                    </div>
                    <div>
                      3. As soon as enough points accumulate, you can get a
                      guaranteed prize!
                    </div>
                    <div>4. Each push increases the chance to get Jackpot!</div>
                  </div>
                </div>
              </div>
            </div>
            <div className="wpck-timer-box">
              <div className="time-left">time left</div>
              <div className="time-sec">23:54:55</div>
            </div>
            <div className="btn-close-x"></div>
          </div>
        </div>
      </div>
    </HelmetProvider>
  );
};

export default React.memo(MainScreen16FortunaRoundMainpopComponent);

import React from "react";

import screenUnipopWavepack10 from "../styles/screen-unipop-wavepack10.scss";

const MainScreen22Unipop04wavepack10Component = () => {
    return (
        <>
            <div className="main">
                <div className="main-bg"></div>

                <div className="header">
                    <div className="res-panel">
                        <div className="res gold">
                            <div className="value">20349 T</div>
                            <div className="mlt-prm">x3212</div>
                            <div className="mlt-tmp">
                                <div className="v">32342</div>
                                <div className="t">12:22</div>
                            </div>
                            <div className="btn-add"></div>
                        </div>
                        <div className="res cri">
                            <div className="value">20349 T</div>
                            <div className="mlt-prm">x3212</div>
                            <div className="mlt-tmp">
                                <div className="v">32342</div>
                                <div className="t">12:22</div>
                            </div>
                            <div className="btn-add"></div>
                        </div>
                        <div className="res mem">
                            <div className="value">20349 T</div>
                            <div className="mlt-prm">x3212</div>
                            <div className="mlt-tmp">
                                <div className="v">32342</div>
                                <div className="t">12:22</div>
                            </div>
                            <div className="btn-add"></div>
                        </div>
                    </div>

                    <div className="vip-btn lvl0">
                        <div className="num">32</div>
                    </div>
                    <div className="shop-btn">
                        <div className="btn-add"></div>
                        <div className="icon"></div>
                    </div>

                    <div className="opt-panel">
                        <div className="btn exit"></div>
                        <div className="btn ops"></div>
                        <div className="btn music"></div>
                        <div className="btn sound"></div>
                    </div>
                </div>

                <div className="left-side">
                    <div className="left-panel">
                        <div className="heroes-list">
                            <div className="dps-dpc-panel">
                                <div className="dpc">
                                    <div className="value">20349</div>
                                    <div className="mlt-prm">x3212</div>
                                    <div className="title">
                                        DPC<div className="icon-dpc"></div>
                                    </div>
                                </div>
                                <div className="dps">
                                    <div className="value">234234T</div>
                                    <div className="mlt-prm">x3212</div>
                                    <div className="title">
                                        DPS<div className="icon-dps"></div>
                                    </div>
                                </div>
                                <div className="dps-dpc-panel-d1"></div>
                                <div className="dps-dpc-panel-d2"></div>
                            </div>

                            <ul className="all-heroes">
                                <ul className="my-heroes">
                                    <li className="hero-card disabled">
                                        <div className="avatar">
                                            <img
                                                className="ava"
                                                src="rc/ava1.png"
                                            />
                                        </div>
                                        <div className="dps">234234 K</div>
                                        <div className="name">
                                            JOHN DOEHOVARIUS
                                        </div>
                                        <div className="lvl">
                                            <div className="n">300</div>
                                        </div>
                                        <div className="btn-buy disabled">
                                            <div className="price gold">
                                                234234
                                            </div>
                                            <div className="txt">BUY</div>
                                        </div>
                                    </li>
                                    <li className="hero-card disabled">
                                        <div className="avatar">
                                            <img
                                                className="ava"
                                                src="rc/ava2.jpg"
                                            />
                                        </div>
                                        <div className="abil-ready"></div>
                                        <div className="dps">234234 T</div>
                                        <div className="name">
                                            JOHN DOEHOVARIUS
                                        </div>
                                        <div className="lvl">
                                            <div className="n">300</div>
                                        </div>
                                        <div className="btn-buy disabled">
                                            <div className="price gold">
                                                234234
                                            </div>
                                            <div className="txt">UPGRADE</div>
                                        </div>
                                    </li>
                                    <li className="hero-card">
                                        <div className="avatar">
                                            <img
                                                className="ava"
                                                src="rc/ava3.jpg"
                                            />
                                        </div>
                                        <div className="abil-ready"></div>
                                        <div className="dps">234234 M</div>
                                        <div className="name">
                                            JOHN DOEHOVARIUS
                                        </div>
                                        <div className="lvl">
                                            <div className="n">1</div>
                                        </div>
                                        <div className="btn-buy">
                                            <div className="price gold">
                                                23423
                                            </div>
                                            <div className="txt">BUY</div>
                                        </div>
                                    </li>
                                    <li className="hero-card">
                                        <div className="avatar">
                                            <img
                                                className="ava"
                                                src="rc/ava4.jpg"
                                            />
                                        </div>
                                        <div className="abil-ready"></div>
                                        <div className="dps">234234 R</div>
                                        <div className="name">
                                            JOHN DOEHOVARIUS
                                        </div>
                                        <div className="lvl">
                                            <div className="n">1123</div>
                                        </div>
                                        <div className="btn-buy">
                                            <div className="price gold">
                                                234234
                                            </div>
                                            <div className="txt">BUY</div>
                                        </div>
                                    </li>
                                </ul>

                                <div className="new-heroes-title">
                                    Earn gold and unlock new heroes
                                </div>

                                <ul className="new-heroes">
                                    <li className="hero-card">
                                        <div className="avatar"></div>
                                        <div className="dps">234234</div>
                                        <div className="name">
                                            JOHN DOEHOVARIUSDSFASDFAS
                                        </div>
                                        <div className="btn-buy">
                                            <div className="price gold">
                                                234234
                                            </div>
                                        </div>
                                    </li>
                                    <li className="hero-card">
                                        <div className="avatar"></div>
                                        <div className="dps">234234</div>
                                        <div className="name">
                                            JOHN DOEHOVARIUS
                                        </div>
                                        <div className="btn-buy">
                                            <div className="price gold">
                                                234234
                                            </div>
                                        </div>
                                    </li>
                                    <li className="hero-card">
                                        <div className="avatar"></div>
                                        <div className="dps">234234T</div>
                                        <div className="name">
                                            JOHN DOEHOVARIUS
                                        </div>
                                        <div className="btn-buy">
                                            <div className="price gold">
                                                234234
                                            </div>
                                        </div>
                                    </li>
                                    <li className="hero-card">
                                        <div className="avatar"></div>
                                        <div className="dps">234234</div>
                                        <div className="name">
                                            JOHN DOEHOVARIUS
                                        </div>
                                        <div className="btn-buy">
                                            <div className="price gold">
                                                234234
                                            </div>
                                        </div>
                                    </li>
                                    <li className="hero-card">
                                        <div className="avatar"></div>
                                        <div className="dps">234234</div>
                                        <div className="name">
                                            JOHN DOEHOVARIUS{" "}
                                        </div>
                                        <div className="btn-buy">
                                            <div className="price gold">
                                                234234
                                            </div>
                                        </div>
                                    </li>
                                </ul>
                            </ul>
                        </div>

                        <div className="skills">
                            <ul className="skill-list">
                                <li className="skill">
                                    <img
                                        className="icon"
                                        src="rc/skill-ico1.png"
                                    />
                                </li>
                                <li className="skill">
                                    <img
                                        className="icon"
                                        src="rc/skill-ico2.png"
                                    />
                                </li>
                                <li className="skill">
                                    <img
                                        className="icon"
                                        src="rc/skill-ico1.png"
                                    />
                                </li>
                                <li className="skill">
                                    <img
                                        className="icon"
                                        src="rc/skill-ico2.png"
                                    />
                                </li>
                                <li className="skill idle">
                                    <img
                                        className="icon"
                                        src="rc/skill-ico1.png"
                                    />
                                    <div className="timer">10:12</div>
                                </li>
                                <li className="skill disabled">
                                    <img
                                        className="icon"
                                        src="rc/skill-ico2.png"
                                    />
                                </li>
                                <li className="skill disabled">
                                    <img
                                        className="icon"
                                        src="rc/skill-ico2.png"
                                    />
                                </li>
                            </ul>
                        </div>

                        <div className="loc-list">
                            {" "}
                            {/* back */}
                            <div className="loc-back">
                                <img className="icon" src="rc/ico-loc.png" />
                                <div className="text">2561</div>
                                <div className="text1">GO BACK</div>
                            </div>
                            <ul className="locs">
                                <li className="loc">
                                    <img
                                        className="icon"
                                        src="rc/ico-loc.png"
                                    />
                                    <div className="text">2556</div>
                                </li>
                                <li className="loc">
                                    <img
                                        className="icon"
                                        src="rc/ico-loc.png"
                                    />
                                    <div className="text">2557</div>
                                </li>
                                <li className="loc">
                                    <img
                                        className="icon"
                                        src="rc/ico-loc.png"
                                    />
                                    <div className="text">2558</div>
                                </li>
                                <li className="loc">
                                    <img
                                        className="icon"
                                        src="rc/ico-loc.png"
                                    />
                                    <div className="text">2559</div>
                                </li>
                                <li className="loc">
                                    <img
                                        className="icon"
                                        src="rc/ico-loc.png"
                                    />
                                    <div className="text">2560</div>
                                </li>
                                <li className="loc current">
                                    <img
                                        className="icon"
                                        src="rc/ico-loc.png"
                                    />
                                    <div className="text">2561</div>
                                </li>
                            </ul>
                        </div>
                    </div>
                    <ul className="tabs">
                        <li className="heroes selected"></li>
                        <li className="shop"></li>
                        <li className="achiev disable"></li>
                        <li className="stat"></li>
                        <li className="guild disable"></li>
                        <li className="album"></li>
                    </ul>
                </div>

                <div className="popup-layer" style="display: block;">
                    <div
                        className="screen-blend-55"
                        style="display: block;"></div>

                    <div
                        className="m-popup unipop-wavepack10"
                        style="display: block;">
                        <div className="title">annoucement</div>
                        <div className="color-box"></div>
                        <div className="color-box-decor"></div>
                        <div className="unipop-box-content-all">
                            <div className="unipop-box-left-box">
                                <div className="unipop-scroll-box">
                                    <div className="unipop-box select">
                                        <div className="unipop-box-pic">
                                            <img src="rc/unipop-m-wom-pack.jpg" />
                                        </div>
                                        <div className="unipop-gold-arrow"></div>
                                    </div>
                                    <div className="unipop-box select">
                                        <div className="unipop-box-pic">
                                            <img src="rc/unipop-m-sed-pack.jpg" />
                                        </div>
                                        <div className="unipop-gold-arrow"></div>
                                    </div>
                                    <div className="unipop-box select">
                                        <div className="unipop-box-pic">
                                            <img src="rc/unipop-m-lov-pack.jpg" />
                                        </div>
                                        <div className="unipop-gold-arrow"></div>
                                    </div>
                                    <div className="unipop-box select">
                                        <div className="unipop-box-pic">
                                            <img src="rc/unipop-m-don-pack.jpg" />
                                        </div>
                                        <div className="unipop-gold-arrow"></div>
                                    </div>
                                    <div className="unipop-box select">
                                        <div className="unipop-box-pic">
                                            <img src="rc/unipop-m-plb-pack.jpg" />
                                        </div>
                                        <div className="unipop-gold-arrow"></div>
                                    </div>
                                    <div className="unipop-box select">
                                        <div className="unipop-box-pic">
                                            <img src="rc/unipop-m-cas-pack.jpg" />
                                        </div>
                                        <div className="unipop-gold-arrow"></div>
                                    </div>
                                    <div className="unipop-box">
                                        <div className="unipop-box-pic">
                                            <img src="rc/unipop-m-fortuna.jpg" />
                                        </div>
                                        <div className="unipop-gold-arrow"></div>
                                    </div>
                                </div>
                            </div>
                            <div className="unipop-box-right-box">
                                <div className="right-box-f">
                                    <div className="right-box-f-name-box">
                                        wave pack 10
                                    </div>
                                    <div className="red-timer-box">
                                        <div className="time-text">
                                            TIME LEFT
                                        </div>
                                        <div className="time">23:12:57</div>
                                    </div>
                                    <div className="rew-box-all">
                                        <div className="rew-box">
                                            <div className="item-box">
                                                <img src="rc/change-ic-gold.png" />
                                                <div className="item-count">
                                                    6969
                                                </div>
                                            </div>
                                            <div className="item-box">
                                                <img src="rc/change-ic-gold.png" />
                                                <div className="item-count">
                                                    6969
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="rew-girl-box">
                                        <div className="rew-girl-ballon">
                                            <div className="rew-girl-ballon-text">
                                                Master, these items will help
                                                you a lot!
                                            </div>
                                        </div>
                                    </div>
                                    <div className="buy-pack-all-box">
                                        <div className="new-old-price-box">
                                            <div className="old-price">
                                                regular price 30$
                                            </div>
                                            <div className="new-price">
                                                now just for 10$
                                            </div>
                                        </div>
                                        <div className="buy-pack-btn">
                                            buy pack
                                        </div>
                                        <div className="value-box">
                                            <div className="value-num">
                                                999%
                                            </div>
                                            <div className="value-text">
                                                value!
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>

                        <div className="btn-close-x"></div>
                    </div>
                </div>
            </div>
        </>
    );
};

export default React.memo(MainScreen22Unipop04wavepack10Component);
